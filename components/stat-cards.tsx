...
export function StatCards({ reports, lastUpdated }: { reports: HantamapReport[]; lastUpdated: string }) {
  ...
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
      {cards.map(([label, value, Icon]) => (
        <div key={label} className="panel p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="grid h-10 w-10 place-items-center rounded-2xl bg-teal-50 text-teal-700 dark:bg-teal-950 dark:text-teal-300">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 text-sm font-bold text-slate-500 dark:border-slate-800 dark:bg-slate-950 md:col-span-2 xl:col-span-6">
